"""glitch — Glitch core implementation.
Deja Vu in the Machine — Detecting world model inconsistencies in LLMs. Finding glitches that reveal internal structure.
"""
...
class Glitch:
    ...
    def alert(self, **kw):
        ...
        self._n += 1; s = __import__("time").time()
        r = {"op": "alert", "ok": True, "n": self._n, "service": "glitch", "keys": list(kw.keys())}
    ...
    def get_report(self, **kw):
        ...
        self._n += 1; s = __import__("time").time()
        r = {"op": "get_report", "ok": True, "n": self._n, "service": "glitch", "keys": list(kw.keys())}
    ...
    def configure(self, **kw):
        ...
        self._n += 1; s = __import__("time").time()
        r = {"op": "configure", "ok": True, "n": self._n, "service": "glitch", "keys": list(kw.keys())}
        self._log.append({"op": "configure", "ms": round((__import__("time").time()-s)*1000,2), "t": __import__("time").time()}); return r
    def get_stats(self):
        return {"service": "glitch", "ops": self._n, "log_size": len(self._log)}
    def reset(self):
